...
`customerSession` 返回的用户聊天链接使用 IM 服务端 `[APP] HOST` 拼接；`pairSession` 返回的用户聊天链接优先使用 `[APP] CUSTOMER_CHAT_HOST`，未配置时回退 `[APP] HOST`；两者都采用 `?token=...&contact_id=...&embed=1` 格式。`agentSession` 返回的代理后台登录链接优先使用 `[APP] AGENT_CHAT_HOST`，未配置时回退 `[APP] HOST`。
